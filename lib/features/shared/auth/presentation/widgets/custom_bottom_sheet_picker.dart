...
class CustomBottomSheetPicker extends StatefulWidget {
  ...
  const CustomBottomSheetPicker({super.key, required this.title, required this.items, this.initialSelection, required this.onSelected});
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  title: CustomText(
                    text: item,
                    style: TextStyle(fontSize: 16, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal, color: AppColors.black),
                  ),
                  trailing: Icon(
                    isSelected ? Icons.radio_button_checked : Icons.radio_button_unchecked,
                    color: isSelected ? AppColors.cF9A405 : AppColors.cE0E5EC,
                  ),
                  tileColor: isSelected ? AppColors.cF9A405.withValues(alpha: 0.05) : Colors.transparent,
                  onTap: () {
                    setState(() {
                      _selectedItem = item;
                    });
...
Future<void> showCustomPicker({
  ...
  return showModalBottomSheet(
    ...
    builder: (context) {
      return Padding(
        padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom, top: MediaQuery.of(context).padding.top + kToolbarHeight),
        child: CustomBottomSheetPicker(title: title, items: items, initialSelection: initialSelection, onSelected: onSelected),
      );
    },
  );
}
